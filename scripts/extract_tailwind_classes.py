...
def extract_cell_text(cell):
    """Extract text from a table cell, handling multi-line vs inline content"""
    # Check if this cell contains actual text nodes with line breaks
    whitespace_div = cell.find('div', class_='*:whitespace-pre!')
    
    if whitespace_div:
        # Get the code element within
        code_elem = whitespace_div.find('code')
        if code_elem:
            # Check if there are actual text nodes with newlines (not just var elements)
            text_content = []
            for element in code_elem.children:
                if element.string:  # Direct text node
                    text_content.append(element.string)
            
            # Join text nodes and check for actual newlines
            full_text = ''.join(text_content)
            if '\n' in full_text:
                # This has real multi-line content
                return code_elem.get_text(separator='\n').strip().replace('\n', '<br>')
    
    # For all other cases (including inline var elements), use empty separator
